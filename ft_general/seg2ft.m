...
%        cfg_ica.method = 'binica';
      ...
      %run artifact detection on components
      %simple correlation between vertical and horizontal eye chans with
      %all the components
...
%         
  ...
  if strcmpi(exper.eegFileExt,'sbin') || strcmpi(exper.eegFileExt,'raw') || strcmpi(exper.eegFileExt,'egis') || strcmpi(exper.eegFileExt,'mff')
    ...
    elseif strcmp(cfg.continuous,'yes')
      ...
      if ana.useExpInfo
        cfg.eventinfo.trl_order = ana.trl_order;
        cfg.eventinfo.sessionNames = ana.sessionNames{sesNum_orig};
        if ~iscell(cfg.eventinfo.sessionNames)
          cfg.eventinfo.sessionNames = {cfg.eventinfo.sessionNames};
        end
        cfg.eventinfo.sessionNum = ses;
        cfg.eventinfo.phaseNames = ana.phaseNames{sesNum_orig};
        cfg.eventinfo.eventValues = eventValue_orig;
        cfg.eventinfo.prepost = prepost;
      end
      ...
      if isfield(ana,'offsetMSTCP')
          cfg.eventinfo.offsetMSTCP = ana.offsetMSTCP; %include TCP default offset
      end
