...
%
% [data,exper] = mm_ft_loadData_multiSes(cfg,exper,dirs,ana,data_evoked)
%
% Input:
    ...
    for ses = 1:length(exper.sesStr)
        ...
        for evVal = 1:length(ana.eventValues{ses})
            ...
            if exist(inputfile,'file')
                ...
                data.(sesStr).(ana.eventValues{ses}{typ}{evVal}).sub(sub).data.cfg.trl = [];
            end % if
        end % for evVal
        %end % for typ
    end % for ses
...
if strcmp(cfg.baseline_data,'pow')
    if ~isempty(cfg.baseline_type) && (strcmp(cfg.ftype,'pow') || strcmp(cfg.output,'pow'))
        ...
        if (strcmp(cfg.norm_trials,'average') || strcmp(cfg.norm_trials,'single')) && ~isempty(cfg.baseline_time)
